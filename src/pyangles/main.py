...
class Pyangles:
    ...
    def search(self, df, key, window, order):
        ...
        for f in self.functions:
            data.append({'key': f.__name__, 'value': f.__name__.replace('_' , ' ').capitalize() if f(df, key, window, order) else False})
        return data

    def ascending_triangle(self, df, key, window, order):
    ...
    def descending_channel(self, df, key, window, order):
        lows, highs = self.range_slopes(df, key, window, order)
        return lows[f'{key}_lows_slope'].iloc[-1] < 0 and highs[f'{key}_highs_slope'].iloc[-1] < 0 and math.isclose(lows[f'{key}_lows_slope'].iloc[-1], highs[f'{key}_highs_slope'].iloc[-1])

    def ascending_wedge(self, df, key, window, order):
        lows, highs = self.range_slopes(df, key, window, order)
    ...
    def descending_wedge(self, df, key, window, order):
        lows, highs = self.range_slopes(df, key, window, order)
